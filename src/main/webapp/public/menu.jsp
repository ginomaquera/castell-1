<div class="col-sm-3 col-md-2 sidebar">
    
    <ul class="${rol eq 'ADMIN'?'nav nav-sidebar':'hidden'}">
        <li><a href="<%=request.getContextPath()%>/adm/admin">Administradores</a></li>
        <li class="active"><a href="<%=request.getContextPath()%>/adm/vendedor">Vendedores</a></li>
    </ul>
   
    <ul class="nav nav-sidebar">
        <li><a href="<%=request.getContextPath()%>/contacto">Contactos</a></li>
        <li><a href="<%=request.getContextPath()%>/campain">Campañas</a></li>
        <li><a href="<%=request.getContextPath()%>/categoria">Categorías</a></li>        
    </ul>
    
     <ul class="nav nav-sidebar">
        <li><a href="<%=request.getContextPath()%>/prospecto">Prospectos</a></li>
    </ul>
</div>